...
const startGrid = lines.findIndex(l => l.includes("gridTemplateColumns: 'repeat(5, 1fr)'"));
const endGrid = lines.findIndex((l, i) => i > startGrid && l.includes('</section>'));

if (startGrid !== -1 && endGrid !== -1) {
  // Let's just rewrite the whole grid part safely by replacing exactly what's inside.
  // Using line indexing:
  let newBlock = \`            <div style={{ position: 'absolute', inset: '15% 6% 15% 6%', display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gridTemplateRows: 'repeat(2, 1fr)', zIndex: 10 }}>
    ...
    // The original block ends at the closing div of the grid
    const startReplace = startGrid;
    const endReplace = lines.findIndex((l, i) => i > startGrid && l.includes('</section>')) - 2; // lines up to the overlay div
    
    // Actually, I can just replace lines[552] to lines[591]
