...
const s1 = code.indexOf(`{/* Exclusive Policies */}`);
const e1 = code.indexOf(`</div>\n          </div>\n        </motion.div>`, s1);

if (s1 === -1 || e1 === -1) {
  console.log('Not found in Community.tsx', s1, e1);
  process.exit(1);
}

const replacement = `{/* Exclusive Policies */}
...
console.log('Done rewriting Community.tsx policies');
