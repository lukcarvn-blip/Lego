...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  const skipLen = code.substring(endIdx).indexOf('</div>') + 6 + 1; // get past </div> and \n
  const endAnchorActual = `                </div>\r\n              </div>`;
  const skipLen2 = endAnchorActual.length;

  code = code.substring(0, startIdx) + newBlock + code.substring(endIdx + skipLen2);
