...
// If still not found, just search for onMouseLeave
if (startIdx === -1) {
  startIdx = code.indexOf(`                onMouseLeave={() => setCraftHovered(false)}\n              >`);
  if (startIdx !== -1) {
    // move back to <div
...
const endAnchor = `                </div>\n              </div>`;
let endIdx = code.indexOf(endAnchor.replace(/\\n/g, '\\r\\n'), startIdx);
if (endIdx === -1) endIdx = code.indexOf(endAnchor, startIdx);
if (endIdx === -1) {
    // search for the </div> that closes the progress bar container, which is followed by </div> that closes the card body
