...
const startStr = `  const formatPrice = (priceUSD: number, discountPercentage?: number) => {`;
const startIdx = code.indexOf(startStr);

if (startIdx !== -1) {
  // Find the end of formatPrice. It's right before `  // Mock data for Profile Page`
  let endIdx = code.indexOf('  // Mock data for Profile Page', startIdx);
  if (endIdx !== -1) {
...
`;
    code = code.substring(0, startIdx) + newPrice + code.substring(endIdx);
    fs.writeFileSync('src/context/StoreContext.tsx', code, 'utf8');
    console.log("Success with mock data anchor");
  } else {
    console.log("Could not find mock data anchor");
  }
} else {
