...
export const Admin = () => {
  const { 
    ...
    settings, updateSettings, previewSettings, setPreviewSettings,
    t, language, showToast, formatPrice,
    appUsers, currentUserRole, updateUserRole, deleteUser, user, logout, loginWithGoogle, reviews, updateReviewStatus, deleteReview } = useStore();
  ...
  const [activeTab, setActiveTabState] = useSessionState<'dashboard' | 'orders' | 'products' | 'printers' | 'blog' | 'files' | 'members' | 'reviews' | 'settings' | 'more'>('admin_tab', initialTab);
  ...
  const [productSubTab, setProductSubTab] = useSessionState<'list' | 'collections' | 'sizes'>('admin_productSubTab', 'list');
