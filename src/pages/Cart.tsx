              ...
              {cart.map((item) => (
                <div key={`${item.product.id}-${item.size}-${item.material}`} className="glass-panel cart-item-card">
                  <div className="cart-item-img-wrapper">
                    <img src={item.product.images?.[0] || '/images/fallback-logo.jpg'} onError={(e) => { e.currentTarget.src = '/images/fallback-logo.jpg'; }} alt={item.product.name[language]} style={{ width: '100%', height: '100%', objectFit: 'contain', filter: 'drop-shadow(0 10px 10px rgba(0,0,0,0.5))' }} />
                  </div>
                  <div style={{ flex: 1 }}>
